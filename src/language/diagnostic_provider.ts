...
export type DiagnosticProvider<T, D extends MarkerData> = {
     ...
     */
    newRequest: (filename: string, model: TextModel, text: string, charPos: number) => Promise<DiagnosticTask<T>[]>,
     ...
     */
    mergeData: (model: TextModel, currentData: T[], newBatch: T[], previousBatch: T[], currentMarkers: D[]) => DiagnosticMergeResult<T, D>;
}
    ...
    public async updateMarkers(filename: string, model: TextModel, charPos: number): Promise<void> {
        ...
        for (let i = 0; i < len; i++) {
            ...
            const mergeResult = this.provider.mergeData(
                model,
                this.cachedData, newBatch, 
                previousBatch, 
                this.cachedMarkers);
            this.cachedData = mergeResult.nextData;
            this.cachedMarkers = mergeResult.nextMarkers;
            monaco.editor.setModelMarkers(
                model, this.provider.ownerId, this.cachedMarkers);
            previousBatch = newBatch;
        }
        if (len > 0) {
            const mergeResult = this.provider.mergeData(
                model,
                this.cachedData, this.cachedData, this.cachedData, this.cachedMarkers);
            this.cachedData = mergeResult.nextData;
            this.cachedMarkers = mergeResult.nextMarkers;
